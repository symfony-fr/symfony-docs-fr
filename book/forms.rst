...
d'un objet et de les traduire dans un format adapté pour être rendu dans
un formulaire HTML.
...
* ``max_length`` : Si le champ est de type texte, alors l'option ``max_length``
  ...
  est utilisé) ou grâce aux métadonnées de Doctrine (via la longueur du champ).
...
Quelques types de champ ont des options de rendues supplémentaires
qui peuvent être passés au widget. Ces options sont documentées avec chaque
