...
.. note::
    ...
    mapper vos objets avec une base de données relationnelle (comme *MySQL*, *PostGesSQL*
...
.. sidebar:: Un peu plus sur ``doctrine:generate:entities``
    ...
    leurs méthodes getter et setter associé.
...
en vous basant sur la valeur de son ``id`` :
...
#. Modifier l'objet;
#. Appeler la méthode ``flush()`` du gestionnaire d'entités
...
.. note::
    ...
    avec quelques flexibilités supplémentaires. Si ça vous dérange, ne vous
...
.. tip::
    ...
    définies dans la même classe. Pour lier une entité définie dans une autre
...
à l'objet ``Category`` donné via leurs valeurs ``category_id``.
...
Désormais, vous pouvez dire à Doctrine d'exécuter une méthode à n'importe
...
appellera automatiquement la méthode et le champ ``created`` sera défini
à la date courante.
...
.. sidebar:: Callbacks du cycle de vie et traitants d'évènements
    ...
    pour écouter ou vous abonner aux évènements, puis lui donner les accès
...
.. note::
    ...
    Il existe d'autres options qui ne sont pas listées ici. Pour plus de détails,
